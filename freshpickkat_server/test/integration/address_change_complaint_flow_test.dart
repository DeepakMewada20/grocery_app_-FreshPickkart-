...
void main() {
  withServerpod('Address change complaints', (sessionBuilder, endpoints) {
    test(
      'stores address change request metadata on complaint creation',
      () async {
        ...
        try {
          ...
          expect(
            complaint.status,
            equals(PostgresComplaintService.pendingStatus),
          );
          expect(
            complaint.complaintType,
            equals(PostgresComplaintService.deliveryType),
          );
          expect(
            complaint.selectedField,
          ...
          final currentAddress =
              jsonDecode(complaint.extraData!['currentAddressJson']!)
                  as Map<String, dynamic>;
          final requestedAddress =
              jsonDecode(complaint.extraData!['requestedAddressJson']!)
                  as Map<String, dynamic>;
    ...
    test(
      'updates order address and tracking metadata before out for delivery',
      () async {
        final seed = await _seedOrder(
          sessionBuilder,
          endpoints,
    ...
    test(
      'approves address change complaints by updating order and audit log',
      () async {
        final seed = await _seedOrder(
          sessionBuilder,
          endpoints,
        ...
        try {
          ...
          expect(
            updated.status,
            equals(PostgresComplaintService.resolvedStatus),
          );
          ...
          final audit = await protocol.AdminAuditLogRow.db.findFirstRow(
            session,
            where: (t) =>
                t.action.equals('approve_address_change') &
                t.entityType.equals('complaint'),
          );
          expect(audit, isNotNull);
          expect(
            audit!.metadata?['orderNumber'],
            equals(seed.order.orderNumber),
          );
          expect(
            audit.metadata?['selectedField'],
            equals(PostgresComplaintService.addressChangeField),
          );
        } finally {
          await session.close();
    ...
    test(
      'rejects address change complaints with a required reason and audit log',
      () async {
        final seed = await _seedOrder(
          sessionBuilder,
          endpoints,
        ...
        try {
          ...
          final complaint = await complaintService.createDeliveryComplaint(
            ...
            title: 'Delivery Location Issue',
            description:
                'Reject this change because the rider is already nearby.',
            selectedField: PostgresComplaintService.addressChangeField,
          ...
          expect(
            updated.status,
            equals(PostgresComplaintService.rejectedStatus),
          );
          ...
          final audit = await protocol.AdminAuditLogRow.db.findFirstRow(
            session,
            where: (t) =>
                t.action.equals('reject_address_change') &
                t.entityType.equals('complaint'),
          );
          expect(audit, isNotNull);
    ...
    test(
      'filters paginated complaint lists down to address change requests',
      () async {
        ...
        try {
          ...
          expect(
            {
              firstPage.complaints.first.orderNumber,
              secondPage.complaints.first.orderNumber,
            },

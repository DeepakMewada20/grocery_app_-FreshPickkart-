...
class FeaturedVariantResolver {
  ...
  /// 2. Combo Offer — first available variant with `comboOfferIds`
  /// 3. Free Delivery — first available variant with `isFreeDelivery`
  /// 4. Shop More, Get More — first available variant with `shopMoreGetMoreOfferId`
  /// 5. Discount — first available variant with `realPrice > price`
  /// 6. Default — first available variant, else the first variant
  ///
  ...
  static ProductVariant resolve(List<ProductVariant> variants) {
    ...
    // Priority 4: Shop More, Get More
    final smgm = _firstWithOffer(
      variants,
      (v) => v.shopMoreGetMoreOfferId?.isNotEmpty == true,
    );
    if (smgm != null) return smgm;

    // Priority 5: Discount (any variant with sale price below list price)
    ...
    // Priority 6: first available variant, else the first variant
